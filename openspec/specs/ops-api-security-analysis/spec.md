...
The system MUST provide an endpoint that accepts tfvars JSON input and returns security findings.

#### Scenario: Analyze valid tfvars payload
- **WHEN** a client calls `POST /v1/security/analyze-tfvars` with valid tfvars JSON
...
The system MUST persist findings and provide query access with status and severity filtering.

#### Scenario: Query open findings
- **WHEN** a client calls `GET /v1/security/findings?status=open`
